        ...
        if (rand() % 100 < 10) { // 10% chance to clone
            for (size_t i = 0; i < team.size(); i++) {
                if (team[i]->hp > 0 && (dynamic_cast<LightInfantry*>(team[i].get()) || dynamic_cast<Archer*>(team[i].get()))) {
                    ...
                    cloned->position = team[i]->position;
...
public:
    ...
    // Moved createTeam into GameManager
    ...
    // Moved simulateRound into GameManager
    void simulateRound(vector<unique_ptr<Unit>>& team1, vector<unique_ptr<Unit>>& team2,
                       const string& team1Name, const string& team2Name, int round) {
        cout << "\nRound " << round << ":\n";
        // Team 1 actions
        for (size_t i = 0; i < team1.size(); i++) {
            if (team1[i]->hp <= 0) continue;
            team1[i]->specialAbility(team1, team1Name);
            if (team2.empty()) break;
...
int main() {
    ...
    gm->createTeam(team1, team1Name, 100); // Call via GameManager
    gm->createTeam(team2, team2Name, 100); // Call via GameManager
    system("clear");
    ...
    while (gm->isTeamAlive(team1) && gm->isTeamAlive(team2)) {
        gm->simulateRound(team1, team2, team1Name, team2Name, round++); // Call via GameManager
