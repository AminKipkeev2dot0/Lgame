...
1. **Одиночка (Singleton)**  
   - **Класс**: `GameManager`.  
   - **Описание**: Гарантирует, что существует только один экземпляр менеджера игры для управления состоянием.  
...
1. **createTeam**  
   - Выводит название команды и баланс (100).  
   - Циклически запрашивает типы юнитов, пока баланс не исчерпан или не введено "done".  
   - Использует `UnitFactory` для создания юнитов, вычитает их стоимость и добавляет в команду.  
   - Выводит разделительную линию.

2. **simulateRound**  
   - Выводит номер раунда.  
   - Для каждой команды:  
     - Проходит по юнитам, пропуская мертвых (HP ≤ 0).  
     - Активирует `specialAbility`.  
     - Лучники атакуют врагов в пределах 3 позиций, остальные — только позицию 1, если сами на позиции 1.  
     - Прекращает действия, если вражеская команда пуста.
